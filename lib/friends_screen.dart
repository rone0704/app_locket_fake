...
class _FriendsScreenState extends State<FriendsScreen> {
  ...
  Future<void> _acceptRequest(String senderEmail) async {
    try {
      ...
      // 2. Cập nhật phía HỌ
      var senderQuery = await FirebaseFirestore.instance.collection('users').where('email', isEqualTo: senderEmail).get();
      if (senderQuery.docs.isNotEmpty) {
        String senderUid = senderQuery.docs.first.id;
        await FirebaseFirestore.instance.collection('users').doc(senderUid).update({
          'friends': FieldValue.arrayUnion([currentUser!.email])
        });
      }
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Đã kết bạn với $senderEmail")));
    } catch (e) {
      print(e);
    }
  }
  ...
  void _removeFriend(String friendEmail) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Bạn có chắc muốn xóa $friendEmail?", style: const TextStyle(color: Colors.white70)),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF1C1C1E), // Màu nền xám đen chuẩn iOS dark mode
      appBar: AppBar(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (data == null) return const Center(child: Text("Lỗi dữ liệu", style: TextStyle(color: Colors.white)));
          ...
          return SingleChildScrollView(
            child: Column(
              children: [
                ...
                Text("${friends.length} / 20 người bạn", style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                const SizedBox(height: 5),
                const Text("Thêm các bạn thân của bạn", style: TextStyle(color: Colors.grey, fontSize: 14)),
                  ...
                  child: Container(
                    ...
                    child: const Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 10),
                        Text("Thêm một người bạn mới", style: TextStyle(color: Colors.white70, fontSize: 16, fontWeight: FontWeight.w500)),
                      ],
                    ),
                  ),
                ),
                ...
                const Padding(
                  ...
                  child: Align(alignment: Alignment.centerLeft, child: Text("Find friends from other apps", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16))),
                ),
                ...
                if (friends.isNotEmpty) ...[
                  const Padding(
                    padding: EdgeInsets.symmetric(horizontal: 20),
                    child: Align(alignment: Alignment.centerLeft, child: Text("Bạn bè của bạn", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16))),
                  ),
                ...
                if (requests.isNotEmpty) ...[
                  ...
                  const Padding(
                    ...
                    child: Align(alignment: Alignment.centerLeft, child: Row(
                      children: [
                        Icon(Icons.people_alt, color: Colors.white70, size: 20),
                        SizedBox(width: 8),
                        Text("Yêu cầu kết bạn", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                      ],
                    )),
                  ),
                  const SizedBox(height: 10),
